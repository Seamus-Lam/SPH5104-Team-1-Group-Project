...
mean(is.na(mtch)) # no missing data
combined$charlson_comorbidity_index <- charlson$charlson_comorbidity_index[mtch]
...
apsiii <- read.csv('studyCohort_apsiii.csv',header=T,stringsAsFactors=F)
mtch <- match(combined$stay_id,apsiii$stay_id)
mean(is.na(mtch)) # 0% missing data
combined$apsiii <- apsiii$apsiii[mtch]

oasis <- read.csv('studyCohort_oasis.csv',header=T,stringsAsFactors=F)
mtch <- match(combined$stay_id,oasis$stay_id)
mean(is.na(mtch)) # 0% missing data
combined$oasis <- oasis$oasis[mtch]

cor(combined$oasis,combined$apsiii)
...
gcs <- read.csv('studyCohort_GCS.csv',header=T,stringsAsFactors=F)
gcs[,'charttime'] <- as.POSIXct(gcs[,'charttime'], format="%Y-%m-%d %H:%M:%S", tz="UTC")
gcs[,'gcs_motor'] <- as.numeric(gsub('NULL',NA,gcs[,'gcs_motor']))
gcs[,'gcs_verbal'] <- as.numeric(gsub('NULL',NA,gcs[,'gcs_verbal']))
gcs[,'gcs_eyes'] <- as.numeric(gsub('NULL',NA,gcs[,'gcs_eyes']))


combined$lowest_gcs_motor_eyes_first24h <- NA
for(i in 1:nrow(combined)){
	index <- which(gcs$stay_id==combined$stay_id[i] &
		as.numeric(difftime(gcs$charttime, combined$intime[i], units = "days")) <= 1)
	if(length(index)==0) next
	combined$lowest_gcs_motor_eyes_first24h[i] <- min(gcs[index,'gcs_motor'] + gcs[index,'gcs_eyes'],na.rm=T)
}

combined$last_gcs_motor_eyes <- NA
combined$last_gcs_motor <- NA

combined$last_gcs_charttime <- NA
combined[,'last_gcs_charttime'] <- as.POSIXct(combined[,'last_gcs_charttime'], format="%Y-%m-%d %H:%M:%S", tz="UTC")

for(i in 1:nrow(combined)){
	index <- which(gcs$stay_id==combined$stay_id[i])
	tmp <- na.omit(gcs[index,-5])
	index = which(tmp$charttime==max(tmp$charttime))
	if(length(index)==0) next
	#combined$last_gcs_motor_eyes[i] <- tmp[index,'gcs_motor'] + tmp[index,'gcs_eyes']
	combined$last_gcs_motor[i] <- tmp[index,'gcs_motor']
	#combined$last_gcs_charttime[i] <- tmp[index,'charttime']
}

combined$first_gcs_motor_eyes <- NA
combined$first_gcs_charttime <- NA
combined[,'first_gcs_charttime'] <- as.POSIXct(combined[,'first_gcs_charttime'], format="%Y-%m-%d %H:%M:%S", tz="UTC")

for(i in 1:nrow(combined)){
	index <- which(gcs$stay_id==combined$stay_id[i])
	tmp <- na.omit(gcs[index,-5])
	index = which(tmp$charttime==min(tmp$charttime))
	if(as.numeric(difftime(tmp$charttime[index], combined$intime[i], units = "days")) > 1) next
	if(length(index)==0) next
	combined$first_gcs_motor_eyes[i] <- tmp[index,'gcs_motor'] + tmp[index,'gcs_eyes']
	combined$first_gcs_charttime[i] <- tmp[index,'charttime']
}
